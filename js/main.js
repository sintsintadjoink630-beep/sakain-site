...
// ===========================================================
// 商品ギャラリー＋ライトボックス（複数ギャラリー対応）
// ===========================================================
window.SakaiGallery = (function () {
  // ギャラリーごとにデータを保持（同一ページで複数呼び出し対応）
  var galleries = {};
  var activeId = null;
  var productIndex = 0;
  var imageIndex = 0;
  ...
  function render(containerId, productList, base) {
    galleries[containerId] = { products: productList, basePath: base };
    var container = document.getElementById(containerId);
    if (!container) return;

    container.innerHTML = productList
      .map(function (p, i) {
        var thumb = encodeURI(base + p.images[0]);
        return (
          ...
          '<img src="' + thumb + '" alt="' + p.name + '" loading="lazy"></button>' +
          '<figcaption class="sr-only">' + p.name + "</figcaption>" +
          "</figure>"
        );
      })
      .join("");
    ...
    container.querySelectorAll(".product-thumb").forEach(function (btn) {
      btn.addEventListener("click", function () {
        open(btn.getAttribute("data-gallery"), parseInt(btn.getAttribute("data-index"), 10));
      });
    });
  ...
  function nav(dir) {
    var p = galleries[activeId].products[productIndex];
    imageIndex = (imageIndex + dir + p.images.length) % p.images.length;
    update();
  }

  function update() {
    var gallery = galleries[activeId];
    var p = gallery.products[productIndex];
    var lightbox = document.getElementById("lightbox");
    var img = lightbox.querySelector(".lightbox-img");
    var prevBtn = lightbox.querySelector(".lightbox-nav.prev");
    var nextBtn = lightbox.querySelector(".lightbox-nav.next");

    img.src = encodeURI(gallery.basePath + p.images[imageIndex]);
    img.alt = p.name;
    lightbox.querySelector(".lightbox-title").textContent = p.fullName;
    lightbox.querySelector(".lightbox-desc").textContent = p.desc;
    lightbox.querySelector(".lightbox-dots").innerHTML = p.images
      .map(function (_, i) {
